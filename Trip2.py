...
# Read Dataset
df = pd.read_csv("1. Transaction in GIS/STP/trip2_ABM.csv")
df.head()
df["Longitude"] = df["Longitude"][::-1]

df2 = pd.read_csv("STP/trip2_LCPA.csv")
df2.head()
df2["Longitude"] = df2["Longitude"][::-1]

# Read the basemap
img = skimage.io.imread('STP/trip2_basemap.png')
...
# Page Layout
layout = go.Layout(
    title='Comparsion of Overland Routes and waterways to Dongwu',
    font_family='Balto',
    width=1000,
    height=800,
    )
...
fig = go.Figure(data=[surf, ABM, LCPA], layout=layout)
